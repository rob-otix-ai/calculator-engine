 ...
 */
import { hashSeed, mulberry32 } from './return-sampler.js';
 ...
 */
export function gompertzSurvival(age, modal, b) {
 ...
 */
export function gompertzConditionalSurvival(age, currentAge, modal, b) {
    if (age <= currentAge)
        return 1.0;
 ...
 */
export function gompertzMedian(modal, dispersion) {
 ...
 */
export function gompertzSampleAge(current_age, modal, b, u) {
...
// Sampler builder
// ===========================================================================
export function buildLongevitySampler(model, seed) {
    if (model.kind === 'Fixed') {
    ...
    const sex = model.sex === 'Unspecified' ? 'F' : model.sex;
    const survArr = sex === 'M' ? tbl.survival.M : tbl.survival.F;
    const ages = tbl.ages;
    function survAt(age) {
        if (age <= ages[0])
            return 1;
    ...
    function conditionalSurv(age, current_age) {
        if (age <= current_age)
            return 1;
    ...
    function inverseCDF(current_age, u) {
        ...
        for (let a = current_age; a <= ages[ages.length - 1]; a++) {
            if (conditionalSurv(a, current_age) <= target)
                return a;
        }
    ...
    return {
        ...
        median(current_age) {
            ...
            for (let a = current_age; a <= ages[ages.length - 1]; a++) {
                if (conditionalSurv(a, current_age) <= 0.5)
                    return a;
            }
            return ages[ages.length - 1];
        },
