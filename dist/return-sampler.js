...
export const SHILLER_SERIES = shillerJson.rows;
// ===========================================================================
 ...
 * not spuriously fail.
 */
export function cholesky(matrix, tol = 1e-10) {
    const n = matrix.length;
    if (n === 0)
        return [];
    for (let i = 0; i < n; i++) {
        if (matrix[i].length !== n) {
            const err = new Error('cholesky: matrix is not square');
    ...
    for (let i = 0; i < n; i++) {
        for (let j = 0; j <= i; j++) {
            ...
            if (i === j) {
                ...
                if (pivot < -tol) {
                    const err = new Error(`Correlation matrix is not positive semi-definite (pivot ${pivot.toExponential(3)} at row ${i}).`);
                    err.code = 'NON_PSD_CORRELATION';
 ...
 */
export function buildCovariance(assetClasses, correlation) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        if (ai === undefined) {
            throw new Error(`correlation matrix is missing id "${assetClasses[i].id}"`);
        }
        for (let j = 0; j < n; j++) {
            const aj = idIndex.get(assetClasses[j].id);
            if (aj === undefined) {
                throw new Error(`correlation matrix is missing id "${assetClasses[j].id}"`);
            }
...
export function hashSeed(seed, year) {
...
export function studentTCDF(x, dof) {
    const v = dof;
    const xx = (v) / (v + x * x);
 ...
 */
export function inverseStudentT(p, dof) {
    if (p <= 0)
        return -Infinity;
    if (p >= 1)
        return Infinity;
 ...
 */
export function buildReturnSampler(assetClasses, correlation, process, seed) {
    if (assetClasses.length === 0) {
        throw new Error('buildReturnSampler: assetClasses must be non-empty');
    }
    if (process.kind === 'StudentT' && process.dof <= 2) {
        const err = new Error(`Student-T degrees of freedom must be > 2 for finite variance (got ${process.dof}).`);
    ...
    if (process.kind === 'Bootstrap') {
        ...
        if (yLo >= yHi) {
            const err = new Error(`bootstrap window invalid: ${yLo}-${yHi}`);
            err.code = 'BOOTSTRAP_WINDOW_INVALID';
            throw err;
        }
        const window = SHILLER_SERIES.filter((r) => r.year >= yLo && r.year <= yHi);
        if (window.length === 0) {
            const err = new Error(`bootstrap window contains no rows (${yLo}-${yHi})`);
            err.code = 'BOOTSTRAP_WINDOW_INVALID';
            throw err;
        }
        return {
...
const EQUITY_LIKE = new Set([
    'us_equity',
    'intl_equity',
    'reit',
    'commodities',
]);
const BOND_LIKE = new Set(['us_bond', 'intl_bond']);
function inflationCorrelationFor(id, returnInflationCorr, bondInflationCorr) {
    if (BOND_LIKE.has(id))
        return bondInflationCorr;
    if (EQUITY_LIKE.has(id))
        return returnInflationCorr;
 ...
 */
export function buildJointSampler(assetClasses, correlation, process, inflationProcess, returnInflationCorr, bondInflationCorr, seed) {
        ...
        return {
            sample(year, _priorInflation) {
                ...
                for (let i = 0; i < n; i++) {
                    const id = assetClasses[i].id;
                    if (EQUITY_LIKE.has(id))
                        ret[id] = row.us_equity;
                    else if (BOND_LIKE.has(id))
                        ret[id] = row.us_bond;
                    else if (id === 'cash')
                        ret[id] = means[i];
                    else
    ...
    const aug = baseCov.map((row) => [...row, 0]);
    aug.push(new Array(n + 1).fill(0));
    for (let i = 0; i < n; i++) {
        const corr = inflationCorrelationFor(assetClasses[i].id, returnInflationCorr, bondInflationCorr);
    ...
    const studentScale = dof > 2 ? Math.sqrt(dof / (dof - 2)) : 1;
    return {
        sample(year, priorInflation) {
            const rng = mulberry32(hashSeed(seed, year));
            const z = new Array(n + 1);
            for (let i = 0; i < n + 1; i++)
                z[i] = standardNormal(rng);
            const correlatedZ = new Array(n + 1).fill(0);
            for (let i = 0; i < n + 1; i++) {
                for (let j = 0; j <= i; j++)
                    correlatedZ[i] += L[i][j] * z[j];
            }
