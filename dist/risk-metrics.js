 ...
 */
export function computeDrawdown(path) {
    if (path.length === 0)
        return { maxDrawdownPct: 0, recoveryYears: Infinity };
    let peak = path[0];
    let peakIdx = 0;
    let worstDDPct = 0;
 ...
 */
export function computeSortino(returns, mar) {
    if (returns.length === 0)
        return NaN;
 ...
 */
export function computeRiskMetrics(inputs, scenario) {
    var _a;
    const { terminal_distribution, real_balance_paths, annualised_returns } = inputs;
    const mar = ((_a = scenario.risk_free_rate_pct) !== null && _a !== void 0 ? _a : 3.5) / 100;
    ...
    const cvar95 = sortedTerminals.slice(0, tail95Count).reduce((a, b) => a + b, 0) / tail95Count;
    const cvar99 = sortedTerminals.slice(0, tail99Count).reduce((a, b) => a + b, 0) / tail99Count;
    ...
    const maxDD = drawdowns.reduce((acc, dd) => (dd.maxDrawdownPct > acc.maxDrawdownPct ? dd : acc), { maxDrawdownPct: 0, recoveryYears: Infinity });
    ...
    const recoveryYears = drawdowns
        ...
        .sort((a, b) => a - b);
    const medianRecovery = recoveryYears.length === 0
        ? Infinity
    ...
    const maxLen = real_balance_paths.reduce((m, p) => Math.max(m, p.length), 0);
    const p10Path = [];
    const p50Path = [];
    const p90Path = [];
