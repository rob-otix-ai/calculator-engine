...
export function resolveWeights(
  ...
  for (const id of Object.keys(lower.weights)) allIds.add(id);
  for (const id of Object.keys(upper.weights)) allIds.add(id);

  const weights: Record<AssetClassId, number> = {};
  for (const id of allIds) {
    const wLow = lower.weights[id] ?? 0;
    const wHigh = upper.weights[id] ?? 0;
